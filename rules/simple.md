...
> Adapted from [abatilo/vimrc/rules/simple.md](https://github.com/abatilo/vimrc/blob/master/rules/simple.md). The original references a Codex MCP collaborative-planning tool we don't have; "CONSULT Codex" has been replaced with "PAUSE and discuss with the user" throughout. Spirit is unchanged.
...
- WHEN asked to add features, abstractions, or dependencies: PAUSE and ask "Can we ship without this?" before proceeding.
...
- WHEN discussing architecture: PAUSE and ask "Can we prototype this first?"
- PREFER "make it work, then make it right" over upfront planning.
- WHEN facing complex designs: PAUSE and ask "What's the simplest thing that works?"
...
- WHEN heavy mocking emerges (>50% of test code): PAUSE and ask whether an integration test or design change would avoid it.
...
3. CHECK `git notes show <commit>` — prior agents may have left context (see `rules/commit-notes.md`)
4. PAUSE and ask the user if the purpose is unclear

- NEVER delete code just because you don't understand it.
- WHEN unsure about behavior: PAUSE and ask before editing.

---

## Pause-and-Discuss Triggers (Blocking)

These require pausing and discussing with the user **before proceeding**:
...
**Behavior:** Surface the tradeoff, present 1-3 options, wait for user direction before continuing.
